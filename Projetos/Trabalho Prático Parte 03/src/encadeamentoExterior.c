...
int buscar(int ISBN, char *nomeArquivoHash, char *nomeArquivoDados, int M) {
	...
	// Posiciona o cursor no arquivo de hash no endereco correspondente ao compartimento.
	...
	// O endereco no arquivo de livros e' dado por lido->prox.
	seguinte = lido->prox;
	...
	while (seguinte != -1) {
		...
		if (livro->ISBN == ISBN) {
			if (livro->flag == OCUPADO) {
				...
				free(livro);
				free(lido);
				// Retorna o endereco onde o livro foi encontrado.
				return seguinte;
			}
		}
...
int inserir(int ISBN, char *nomeLivro, char* nomeAutor, char* dataPubli, char *nomeArquivoHash, char *nomeArquivoDados, int numRegistros, int M) {
	...
	// 1. Tentativa: Obtencao direta do registro via compartimento hash.
	...
	// 2. Tentativa: Busca do registro na lista de tratamento das colisoes.
	while (seguinte != -1) {
